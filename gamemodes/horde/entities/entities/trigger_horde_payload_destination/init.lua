...
function ENT:StartTouch(ent)
    ...
    if ent:IsPlayer() and ent:Horde_HasPayload() and ent:Alive() then
        ...
        self:Input("ondelivered", self, self) -- why? i don't undestand why it's here
		self:TriggerOutput("ondelivered", ent) -- trigger storred
    end
end
...
function ENT:KeyValue(key, value)
    ...
    elseif key == "group" then
        ...
        elseif key == "ondelivered" then
                self:StoreOutput(key, value) -- store
    end
end