...
function ENT:FlashBang()
    ...
    for _, k in pairs(targets) do
        ...
        elseif k:IsNPC() then
            HORDE:TimeStop_freeze_npc(k, "StunGrenade")
            local timername = "Horde_Ent_" .. k:EntIndex() .. "Stun_Grenade"
            local timerobject = HORDE.Timers:Find(timername)
            if !timerobject then
                timerobject = HORDE.Timers:New({
                    linkwithent = k,
                    timername = timername,
                    func = function(timerobj)
                        HORDE:TimeStop_unfreeze_npc(k, "StunGrenade")
                    end,
                    repetitions = 1,
                    delay = 3.5,
                })
