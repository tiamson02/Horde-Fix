    ...
    if self.item then
        ...
        if self.item.Mind then
            ...
            if MySelf:Horde_HasSpell(self.item.ClassName) then
                ...
                self.sell_btn:SetText(translate.Get("Shop_Sell_For") .. " " .. tostring(math.floor(price * HORDE.sell_item_mult)) .. "$")
        ...
        (self.item.entity_properties.type == HORDE.ENTITY_PROPERTY_BUYABLEPERK and HORDE:GetBuyablePerkLevel(MySelf, self.item.class) > 0)) then
            ...
            self.sell_btn:SetTextColor(Color(255,255,255))
            self.sell_btn:SetText(translate.Get("Shop_Sell_For") .. " " .. tostring(math.floor(HORDE:GetBuyablePerkPrice(MySelf, self.item.class, HORDE:GetBuyablePerkLevel(MySelf, self.item.class) - 1) * HORDE.sell_item_mult)) .. "$")
            self.sell_btn.Paint = function ()
                surface.SetDrawColor(HORDE.color_crimson)
                surface.DrawRect(0, 0, self:GetWide(), 200)
            end
        ...
        elseif MySelf:Horde_GetMoney() < price or MySelf:Horde_GetWeight() < self.item.weight or (not MySelf:Alive()) then
            ...
            if HORDE:IsItemPlacable(self.item) then
                ...
                if drop_entities[self.item.class] then
                    self.sell_btn:SetVisible(true)
                    self.sell_btn:SetTextColor(Color(255,255,255))
                    self.sell_btn:SetText(translate.Get("Shop_Sell_All_For") .. " " .. tostring(math.floor(price * HORDE.sell_item_mult * drop_entities[self.item.class])) .. "$")
                    self.sell_btn.Paint = function ()
                        surface.SetDrawColor(HORDE.color_crimson)
                        surface.DrawRect(0, 0, self:GetWide(), 200)
                    end
